...
static const char *sendstring_commands[] = {
    "git init", "git clone ", "git add ", "git diff ", "git reset --soft ", "git branch --list", "git checkout ", "git remote add ", "git fetch ", "git pull", "git pull upstream ", "git push", "git push -u origin ", "git push --force-with-lease", "git commit ", "git commit -m \": [TFS-]\"", "git status", "git log",
};
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    [_FL] = LAYOUT(
        ...
        //   CAPS       A        S        D        F        G        H        J                K        L        ;        '      ENTER
        _______, RGB_RMOD, RGB_SPD, RGB_VAD, RGB_HUD, RGB_SAD, _______, _______, C_KIKI, _______, _______, _______, _______,
        //   SHIFT      Z        X        C        V        B        N        M        ,        .        /      RSHIFT                               UP
...
const uint8_t PROGMEM ledmap[][RGB_MATRIX_LED_COUNT][3] = {
    [_FL] =
        {//  These remain on base layer rgb to see adjustments ;)
         ...
         __OFF__, ORANGE, GREEN, AZURE, GOLDEN, MAGENT, __OFF__, __OFF__, RED, __OFF__, __OFF__, __OFF__, __OFF__,
    ...
    if (record->event.pressed) {
        switch (keycode) {
            ...
            case G_INIT ... G_LOG:
                ...
                return false;
            case C_KIKI:
                send_string_with_delay("Kiki este cea mai minunata iubita si o iubesc cel mai mult!", 5);
                return false;
            case TOG_NPD:
...
tap_dance_action_t tap_dance_actions[] = {[TD_FN_SWITCH] = ACTION_TAP_DANCE_FN_ADVANCED(NULL, fn_tap_finished, fn_tap_reset)};
